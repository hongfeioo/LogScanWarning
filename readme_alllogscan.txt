...
## 简介
本程序通过过滤日志文件中的关键字，出现“感兴趣”的行，就会触发短信和邮件报警。
...
logfile_path = '/root/log/'                               #日志文件的存放位置，一般情况下是  /var/log/message
...
5. 当配置文件中条目比较多时候，请注意一次运行的时间，合理设置计划任务频率
...
```
vi  /etc/rsyslog.conf                                   #接受日志改变到指定目录

###yihongfei log dir start ###
$template logfile,    "/root/log/%fromhost-ip%"         #定义一个模板
